...
| [`device.rs`](device.rs) | `kurultai connect` endpoints: `POST /api/device/code`, `/api/device/token`, `GET/POST /connect` approval page (CF Access → hub key → loopback gate) | `src/http` · `src/store` · `src/hashutil` | `src/http/mod.rs` | 2026-09-16 | 1 | 2026-09-16 added — seat-scoped agent keys |
| [`device_auth.rs`](device_auth.rs) | Device-code login endpoints (`/auth/device/*`) for hosted agent tokens | `src/http` · `src/store` · `src/hashutil` | `src/http/mod.rs` | 2026-09-16 | 2 | 2026-09-16 `create_device_flow` gains `instance_id`; `human_base_url` shared with `device.rs` · 2026-09-06 added |
| [`hey.rs`](hey.rs) | Multi-agent message board REST (`/api/hey/...`) | `src/store` · `src/brain` · `src/error` | `src/daemon/mod.rs` · `src/http/mod.rs` · `src/http/proposals.rs` · `src/http/ontology_write.rs` | 2026-09-17 | 4 | 2026-09-17 zero-auth solo path: `require_writer` posts as `luke`/local when no auth surface configured · 2026-09-14 `require_agent` reused by ontology_write agent-refusal (#316) · 2026-09-11 `require_agent` now pub(crate) for proposal submit auth (#118) 2026-09-11 `require_agent` now pub(crate) for proposal submit auth (#118) · 2026-09-04 message board endpoints + clamp cleanup |
