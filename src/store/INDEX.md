...
| [`mod.rs`](mod.rs) | Store trait, open_store, SqliteVecStore | `src/error` · `src/hashutil` · `src/memory` · `src/types` | `src/export/mod.rs` · `src/http/ingest.rs` · `src/ontology/mod.rs` · `src/quality/gate.rs` · `src/app/context.rs` · `src/http/device_auth.rs` · `src/http/device.rs` | 2026-09-23 | 8 | 2026-09-23 `atom_epoch()` — mutation epoch on `SqliteVecStore` (upsert/delete/lane/auto-merge bump; `touch_access` doesn't) for prepared `/api/graph` invalidation; `u64::MAX` default = untrackable (#324) · 2026-09-19 `get_thread(id)` added — id-vs-name collision resolution for Hey threads · 2026-09-16 `issue_agent_seat_token`/`revoke_agent`/`deny_device_flow`; key-hash resolve covers seats · 2026-09-15 `classify_atom` in tier paths — sequester rules apply (#325) |
