...
CREATE TABLE IF NOT EXISTS atom_soft_labels (
    atom_id TEXT NOT NULL REFERENCES knowledge_atoms(id) ON DELETE CASCADE,
    label_id INTEGER NOT NULL REFERENCES label_vocab(id) ON DELETE CASCADE,
    score REAL NOT NULL CHECK(score >= 0.0 AND score <= 1.0),
    PRIMARY KEY (atom_id, label_id)
);
...
pub fn migrate(conn: &Connection) -> Result<()> {
    ...
    if current < 5 {
        // Three-tier memory timestamps — backfill last_accessed_at from indexed_at.
        add_column_if_missing(
            conn,
            "knowledge_atoms",
            "last_accessed_at",
        ...
        conn.execute_batch(
            ...
            CREATE INDEX IF NOT EXISTS idx_atoms_last_accessed_at
                ON knowledge_atoms(last_accessed_at DESC);
            "#,
        )
        .map_err(|e| KurultaiError::Store(format!("migration 005 objects failed: {e}")))?;
        conn.execute("INSERT INTO schema_migrations (version) VALUES (?1)", [5])
            .map_err(|e| KurultaiError::Store(format!("migration 005 record failed: {e}")))?;
    }

    if current < 6 {
        conn.execute_batch(MIGRATION_006)
            .map_err(|e| KurultaiError::Store(format!("migration 006 failed: {e}")))?;
        conn.execute("INSERT INTO schema_migrations (version) VALUES (?1)", [6])
            .map_err(|e| KurultaiError::Store(format!("migration 006 record failed: {e}")))?;
    }
