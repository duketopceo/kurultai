...
|------|------|-------|---------|-------|-----|-----------|
| [`auth.rs`](auth.rs) | Hub API-key / admin token gate + CF Access fallback | `src/hashutil` · `src/hub/keys.rs` · `src/http/cf_access.rs` | `src/daemon/mod.rs` | 2026-09-11 | 6 | 2026-09-11 proposals decide route joins SharedClosed write guard (#118) · 2026-09-06 CF Access JWT fallback in `hub_api_auth` · 2026-09-06 exempt `/auth/*` from hub API-key middleware · 2026-08-31 resolve token first; 500 on DB errors · 2026-08-16 indexed (v1 seed) |
...
| [`hey.rs`](hey.rs) | Multi-agent message board REST (`/api/hey/...`) | `src/store` · `src/brain` · `src/error` | `src/daemon/mod.rs` · `src/http/mod.rs` · `src/http/proposals.rs` | 2026-09-11 | 2 | 2026-09-11 `require_agent` now pub(crate) for proposal submit auth (#118) · 2026-09-04 message board endpoints + clamp cleanup |
...
| [`mod.rs`](mod.rs) | Axum daemon: /api/*, /ui/, /auth/*, optional /mcp SSE | `src/brain` · `src/daemon` · `src/error` · `src/mcp` · `src/metrics` · `src/http/hub_listen.rs` · `src/http/device_auth.rs` | `src/daemon/mod.rs` | 2026-09-11 | 7 | 2026-09-11 merge proposals routes; O3 HTTP tests (#118) · 2026-09-06 merge device-auth routes for `kurultai login` · 2026-09-04 add agent_store to HubGate test fixtures · 2026-08-31 validate promote reason length · 2026-08-29 secure unprefixed aliases · 2026-08-16 indexed |
